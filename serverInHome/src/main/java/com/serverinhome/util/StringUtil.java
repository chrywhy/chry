...
public class StringUtil {
    ...
    private static final Pattern mailAddrPattern =
    ...
    /**
     * for masking passwords *
     */
    public static String mask(String s) {
        return mask(s, '*');
    }

    public static String mask(String s, char m) {
    ...
    /**
     * 'str1' is a comma-separated string such as "foo, bar, blah". This
     * methods check if str2 is an element of str1. For example,
     * containsIgnoreCase("foo, bar, blah", "bar") -> true.
     */
    public static boolean containsIgnoreCase(String str1, String str2) {
        String tokens[] = split(str1, ",", true);
    ...
    /**
     * Different from split where 'delim' is treated as a single one
     */
    public static String[] split2(String str, String delim) {
        return split2(str, delim, false);
    }

    public static String[] split2(String str, String delim, boolean trim) {
        ...
        while (idx >= 0) {
            ...
            if (token.length() > 0) {
                arr.add(trim ? token.trim() : token);
            }

            str = str.substring(idx + delimLen);
            idx = str.indexOf(delim);
        }
        if (str.length() > 0) {
            arr.add(trim ? str.trim() : str);
        }
    ...
    /**
     * perl Split like method
     * If num is specified and positive, it represents the maximum number of fields
     * the EXPR will be split into, though the actual number of fields returned
     * depends on the number of times PATTERN matches within EXPR
     *
     * @param trim if true, then trim the tokens
     */
    public static String[] perlSplit(String str, String delim, int num, boolean trim) {
        if (num <= 0) {
            throw new IllegalArgumentException("num cannot be negative or zero");
        }
    ...
    public static String[] perlSplit(String str, String delim, int num) {
        return perlSplit(str, delim, num, false);
    }
    ...
    public static int[] splitInts(String str, String delimiters) throws IllegalArgumentException {
        String[] tokens = splitAndTrim(str, delimiters);
    ...
    public static double[] splitDoubles(String str, String delimiters) {
        String[] tokens = splitAndTrim(str, delimiters);
        double[] list = new double[tokens.length];
        for (int i = 0; i < tokens.length; i++) {
            list[i] = Double.parseDouble(tokens[i]);
        }
        return list;
    }
    ...
    public static String joinInts(Collection<Integer> tokens, String delimiter) {
    ...
    public static String joinDoubles(double[] tokens, String delimiter) {
        if (tokens == null) {
            return "";
        }
        StringBuilder result = new StringBuilder();
    ...
    /**
     * Concatenates the given String[] array into one String, inserting a delimiter
     * between each pair of elements.
     */
    public static String joinStrings(String[] tokens, String delimiter) {
        return joinStrings(tokens, delimiter, 0);
    }

    public static String joinStrings(Collection<String> tokens, String delimiter) {
        return joinStrings(tokens.toArray(new String[0]), delimiter);
    }

    public static String joinStrings(String[] tokens, String delimiter, int start) {
        if (tokens == null) {
            return "";
        }
        StringBuilder result = new StringBuilder();
    ...
    /**
     * Concatenates the elements from start (inclusive) to end (exclusive) of the given
     * String[] array into one String, inserting a delimiter between each pair of elements
     */
    public static String joinStrings(String[] tokens, String delimiter, int start, int end) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < end; i++) {
            if (sb.length() > 0) {
                sb.append(delimiter);
    ...
    public static String joinStrings(String[] tokens1, String[] tokens2, String delimiter) {
        String r1 = joinStrings(tokens1, delimiter);
        String r2 = joinStrings(tokens2, delimiter);
        if (r1.length() == 0 || r2.length() == 0) {
            return r1 + r2;
        }
        else {
            return r1 + delimiter + r2;
        }
    }
    ...
    public static String replaceChars(String str, int beginIdx, char newchar) {
        StringBuilder buf = new StringBuilder(str);
        for (int i = beginIdx; i < buf.length(); i++) {
     ...
     * javaEscape except that it escapes double quote
    ...
    // all keys in props are uppercase
    public static String replaceProperties(String s, Properties props)
            throws IllegalArgumentException {
        return _replaceProperties(s, props, false, false);
    }

    public static String replacePropertiesIgnoreCase(String s, Properties props) {
        Properties p = new Properties();
        for (Object key : props.keySet()) {
            if (key instanceof String) {
                p.put(((String) key).toUpperCase(), props.get(key));
            }
            else {
        ...
        return _replaceProperties(s, p, true, false);
    }

    public static String replacePropertiesIgnoreCase(String s, Properties props, boolean emptyMissingPlaceholder) {
        Properties p = new Properties();
        for (Object key : props.keySet()) {
            if (key instanceof String) {
                p.put(((String) key).toUpperCase(), props.get(key));
        ...
        return _replaceProperties(s, p, true, emptyMissingPlaceholder);
    }

    /**
     * Replaces all placeholders in s with the corresponding values in props. A placeholder
     * has the format ##ABCDEF##. The string between the two ## must be a sequence of
     * alpha, decimal digit, underscore, or dot. If we can't find the value of a placeholder
     * int props, we will leave it alone.
     *
     * @param s     the string
     * @param props the value of all placeholders
     * @return string w/ all placeholders replaced
     * @throws NullPointerException if s or props is null
     */
    private static String _replaceProperties(String s,
                                             Properties props,
                                             boolean upperName,
                                             boolean emptyMissingPlaceholder) {
        ...
        while (bloc < s.length()) {
            char ch = s.charAt(bloc);
            if (ch != '#') {
                tmp.append(ch);
                bloc++;
            }
            else { // ch is '#'
                if (bloc == s.length() - 1) { // we reach the end of the string
                    tmp.append('#');
                    return tmp.toString();
                }
                ...
                if (placeholder.length() == 0 ||
                        bloc == s.length() ||
                        s.charAt(bloc) != '#' ||
                        (bloc < s.length() - 1 && s.charAt(bloc + 1) != '#')) {
                    tmp.append("##").append(placeholder.toString());
                }
                else {
                    bloc += 2;
                    String lname = placeholder.toString();
                    String name = lname;
                    if (upperName) {
                        name = lname.toUpperCase();
                    }
                    String value = props.getProperty(name);
                    if (value == null) {
                        if (emptyMissingPlaceholder) {
                            tmp.append("");
                        }
                        else {
                            tmp.append("##").append(lname).append("##");
                        }
                    }
                    else {
                        tmp.append(value);
    ...
    /**
     * Check if the given character is alpha, decimal digit, or underscore
     *
     * @param ch the character
     * @return true if ch is alpha, digit, or underscore. false otherwise
     */
    ...
    public static ArrayList<String> splitWithWhiteSpace(String text) {
        ArrayList<String> arr = new ArrayList<String>();
        ...
        while (cur < text.length()) {
            ...
            int end = cur + 1;
            while (end < text.length() && text.charAt(end) != delim) {
                end++;
            }

            String s = text.substring(cur, end);
            if (s.charAt(0) == delim) {
                s = s.substring(1);
                int n = s.length() - 1;
                if (s.charAt(n) == delim) {
            ...
            cur = end + 1;
            while (cur < text.length() && (text.charAt(cur) == ' ' || text.charAt(cur) == '\t')) {
                cur++;
            }
        }
    ...
    // TODO: handle the case if a field contains a comma
    public static String csvRemove(String r, String str) {
        String[] fields = split(r, ",", true);
    ...
    public static String csvReplace(String r, String oldField, String newField) {
        String[] fields = split(r, ",", true);
        StringBuffer buf = new StringBuffer("");
        for (String s : fields) {
            if (s.equals(oldField)) {
                s = newField;
            }
            buf.append(s).append(',');
        }
    ...
    public static String csvAdd(String r, String field) {
        String[] fields = split(r, ",", true);
        boolean found = false;
        for (String s : fields) {
            if (s.equals(field)) {
        ...
        if (found) {
            ...
            return fields.length > 0 ? r + "," + field : field;
        }
    }

    // See rfc4180 for spec for CSV.
    public static String escapeCsvString(String s) {
        if (s.indexOf(QUOTE) > 0) {
            s = s.replace(QUOTE, ESCAPED_QUOTE);
        }

        for (String ch : CHARACTERS_THAT_MUST_BE_QUOTED) {
            if (s.indexOf(ch) > -1) {
                s = QUOTE + s + QUOTE;
            }
        }
    ...
    public static String unescapeCsvSring(String s) {
    ...
    public static String formatDecimal(String value) {
        if (value.equalsIgnoreCase("N/A") || value.equalsIgnoreCase("NaN")) {
            return "No data";
        }
    ...
    /**
      * Dump the thread info (name, status, cpu usage percentage, and stack trace into
      * a StringBuilder sb)
      */
     public static String dumpThreadInfo(final ThreadMXBean threadBean,
                                          final Thread thread) {
         ThreadInfo inf = threadBean.getThreadInfo(thread.getId(), 50);
         if (inf == null) {
             return "";
         }
         ...
         sb.append(String.format(
                 "%s %s\n", inf.getThreadName(), inf.getThreadState()
         ));

         StackTraceElement[] stes = inf.getStackTrace();
    ...
    /**
     * covert the long value to k,m, g string style
     * e.g. 1024 -> 1.00k 
     * @param value
     * @return
     */
    public static String toKMG(long value) {
        if (value < 1024) {
            return value + "";
        }
        else if (value < 1024 * 1024) {
            return String.format("%.2fK", value / 1024.00);
        }
        else if (value < 1024 * 1024 * 1024) {
            return String.format("%.2fM", value / (1024.00 * 1024.00));
        }
        else {
            return String.format("%.2G", value / (1024.00 * 1024.00 * 1024.00));
        }
    }

    //check if mail is valid.
    public static boolean validMailAddr(String mail) {
        if (mail == null) {
            return false;
        }
        else {
            return mailAddrPattern.matcher(mail).matches();
        }
    }
}
